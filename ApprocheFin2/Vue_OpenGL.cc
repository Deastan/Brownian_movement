/**
 * \file Vue_OenGL.cc
 * \brief est la définition de la classe qui crée et lance le système, timer, evolue de notre système, dessione, etc...
...
/*!
     *  \brief Constructeur
     *
     *  Constructeur de la Vue_OenGL 
     *	qui initialise le système, le timer, la position de la caméra au départ
     * 
     */
...
void Vue_OpenGL::dessine(wxPaintEvent&)
{
  ...
  glRotated(phi,   0.0, 0.0, 1.0);
	...
	//appelle de la méthode dessine de systeme 
	systeme.dessine();
...
void Vue_OpenGL::OnKeyDown( wxKeyEvent& event )
{
  ...
  case 'A':
    systeme.ajouterArgon(20, true);
    break;
  
  case 'F':
    systeme.ajouterFluor(1, true);
    break;
  
  case 'H':
    systeme.ajouterHelium(20, true);
    break;
  
  case 'N':
    systeme.ajouterNeon(20, true);
    break;
  }
...
{
  systeme.evolue(event.GetInterval() * 0.0001);
